...
export default function AdminVisitsPage() {
  ...
  const filtered = useMemo(() => {
    let list = visits;
    if (repId) list = list.filter((v) => v.rep_id === repId);
  ...
  async function handleForward(visitId: string) {
    setBusyId(visitId);
    try {
      await forwardVisitToManager(visitId);
      await load();
    } catch (e: any) {
  ...
  async function saveNotes(visitId: string) {
    try {
      await updateAdminNotes(visitId, notesDraft);
      await load();
      setNotesOpen(null);
      setNotesDraft("");
    } catch (e: any) {
        ...
        <div className="space-y-3">
          {filtered.map((v) => {
            ...
            return (
              <div
                key={v.id}
                className="rounded-2xl bg-white border border-slate-200 p-5 hover:border-slate-300 transition"
              >
                <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                  ...
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-semibold border shrink-0 ${meta.color}`}
                  >
                    {meta.ar}
                  </span>
                </div>
                ...
                {v.status === "COMPLETED" && (
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    ...
                    >
                      <Icon d={sentToAdmin ? icons.check : icons.visits} size={10} />
                      {sentToAdmin ? "تقرير مستلم" : "بانتظار التقرير"}
                    </span>
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold border ${
                        forwarded
                    ...
                    >
                      <Icon d={forwarded ? icons.check : icons.visits} size={10} />
                      {forwarded ? "مُحوَّل للمدير" : "لم يُحوَّل"}
                    </span>
                  </div>
                )}
                ...
                {/* Feedback preview */}
                {v.feedback_positive && (
                  <div className="rounded-xl bg-teal-50/50 border border-teal-100 p-3 mb-3">
                    <div className="text-[10px] font-semibold text-teal-900 mb-1">
                      إيجابيات
                    </div>
                ...
                {v.feedback_objections && (
                  <div className="rounded-xl bg-amber-50/50 border border-amber-100 p-3 mb-3">
                ...
                {v.admin_notes && notesOpen !== v.id && (
                  <div className="rounded-xl bg-sky-50 border border-sky-100 p-3 mb-3">
                    <div className="text-[10px] font-semibold text-sky-900 mb-1">
                      ملاحظاتي
                    </div>
                    <p className="text-xs text-sky-800">{v.admin_notes}</p>
                  </div>
                )}
                ...
                {notesOpen === v.id ? (
                  <div className="space-y-2 mb-3">
                    <textarea
                      ...
                      rows={3}
                      placeholder="اكتب ملاحظاتك على التقرير..."
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-2 rounded-lg bg-slate-900 text-white text-xs font-medium hover:bg-slate-800"
                      >
                        حفظ
                      </button>
                      <button
                        ...
                        className="px-4 py-2 rounded-lg bg-white border border-slate-200 text-slate-700 text-xs font-medium"
                      >
                        إلغاء
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="text-[11px] font-medium text-sky-600 hover:text-sky-800 mb-3"
                  >
                    {v.admin_notes ? "تعديل الملاحظات" : "+ إضافة ملاحظات"}
                  </button>
                )}

                {v.status === "COMPLETED" && sentToAdmin && !forwarded && (
                  <button
                    onClick={() => handleForward(v.id)}
                    disabled={busyId === v.id}
                    className="w-full py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-xs font-bold shadow-md shadow-indigo-500/20 hover:from-indigo-600 hover:to-purple-600 transition disabled:opacity-60"
                  >
