...
import { apiFetch, getAnalyticsOverview, getVisits, getMyReps, User, AnalyticsOverview, Visit } from "@/lib/api";
...
export default function ManagerDashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [meData, overview, repsData, visitsData] = await Promise.all([
  ...
  const QUICK_LINKS = [
    { href: "/manager/team",      label: "الفريق كامل",   icon: icons.users,   color: "from-sky-500 to-cyan-500" },
    { href: "/manager/visits",    label: "كل الزيارات",   icon: icons.visits,  color: "from-indigo-500 to-violet-500" },
    { href: "/manager/analytics", label: "التحليلات",     icon: icons.target,  color: "from-teal-500 to-emerald-500" },
    { href: "/manager/products",  label: "المنتجات",      icon: icons.doctors, color: "from-purple-500 to-pink-500" },
  ];
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-10 p-6">
        ...
        <div className="relative flex items-center gap-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-slate-300 mt-1.5">
              نظرة سريعة على أداء الفريق والأرقام الأساسية
            </p>
          </div>
        </div>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            ...
            <div className="text-xs font-medium text-slate-500 mb-1">إجمالي الزيارات</div>
          ...
          >
            ...
            <div className="text-[10px] text-teal-600 mt-1 font-medium">
              {stats.totals.conversion_rate}% معدل التحويل
            </div>
          </Link>
          ...
          >
            ...
            <div className="text-xs font-medium text-slate-500 mb-1">المندوبين</div>
            <div className="text-3xl font-bold text-slate-900 tabular-nums">
              {reps.length}
            </div>
            <div className="text-[10px] text-slate-400 mt-1">في الشركة</div>
          </Link>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-slate-700">آخر الزيارات</h2>
          <Link
            href="/manager/visits"
