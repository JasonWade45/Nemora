          ...
          {filtered.map((v) => {
            ...
            return (
              <div key={v.id} className="rounded-2xl bg-white border border-slate-200 p-5 hover:border-slate-300 transition">
                ...
                {v.admin_notes && notesOpen !== v.id && <div className="rounded-xl bg-sky-50 border border-sky-100 p-3 mb-3"><div className="text-[10px] font-semibold text-sky-900 mb-1">ملاحظات</div><p className="text-xs text-sky-800">{v.admin_notes}</p></div>}
                {notesOpen === v.id ? (
