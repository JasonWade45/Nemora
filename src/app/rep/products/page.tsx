...
export default function RepProductsPage() {
  ...
  return (
    <div className="space-y-4 pb-4">
        ...
        <div className="relative z-30">
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              {activeCategory && (
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100 shrink-0">
                  {filtered.length}
                </span>
              )}
            </div>
        ...
        <div className="rounded-2xl bg-white border border-slate-200 p-12 text-center">
          ...
          <p className="text-xs text-slate-500 mb-5">
            {search || activeCategory
              ...
              : "ابدأ بإضافة أول منتج ليك"}
          </p>
          {!search && !activeCategory && (
            <button
              onClick={() => setShowForm(true)}
              className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-sm font-semibold shadow-md shadow-indigo-500/20"
            >
      ...
      <Link
        href="/rep"
        className="block text-center text-xs text-slate-500 hover:text-slate-700 pt-2"
      >
        ← رجوع للوحة التحكم
      </Link>
    </div>
